...
def create_borrow_request(
    ...
    if not book:
        raise HTTPException(
            ...
            detail="Không tìm thấy sách được yêu cầu.",
        )
    ...
    borrow_date = (
        ...
        if request_data and request_data.borrow_date
        else datetime.now()
    )
    return_date = (
        request_data.return_date
        if request_data and request_data.return_date
...
def get_all_borrow_requests_for_all_users(
    ...
    borrows = (
        db.query(models.BorrowRequest)
        .options(
            joinedload(models.BorrowRequest.book), joinedload(models.BorrowRequest.user)
        )
        .order_by(desc(models.BorrowRequest.created_at))
        .all()
    )
...
def approve_borrow_request(
    ...
    borrow_request = (
        db.query(models.BorrowRequest).filter(models.BorrowRequest.id == id).first()
    )
    ...
    conflicting_requests = (
        ...
        .filter(
            ...
            func.DATE(models.BorrowRequest.borrow_date)
            <= func.DATE(borrow_request.return_date),
        )
        .all()
    )
...
def get_admin_stats(
    ...
    active_loans = (
        db.query(models.BorrowRequest)
        .filter(models.BorrowRequest.status == "approved")
        .count()
    )
    return {
...
def get_library_stats(
    ...
    most_borrowed = (
        ...
        .filter(
            models.BorrowRequest.status == "approved",
            models.BorrowRequest.borrow_date >= first_day_last_month,
            models.BorrowRequest.borrow_date <= last_day_last_month,
        )
    ...
    most_borrowed_books = [
        {"title": title, "count": count} for title, count in most_borrowed
    ]

    top_borrowers = (
        db.query(
            models.User.full_name, func.count(models.BorrowRequest.id).label("count")
        )
    ...
    top_borrowers_list = [
        {"name": name, "count": count} for name, count in top_borrowers
    ]
